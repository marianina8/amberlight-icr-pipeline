...
func newService(t *testing.T) (*pipeline.Service, map[string]string) {
	...
	ob, _ := router.NewOutbox(t.TempDir())
...
func TestReadTools(t *testing.T) {
	...
	resps := session(t, srv, initMsg,
		...
		call(4, "list_queue", map[string]any{"queue": "delivered"}),
		call(5, "get_item", map[string]any{"id": ids["05-compositing-final-delivered.json"]}),
		call(6, "get_item", map[string]any{"id": "AL-NOPE0000"}),
		call(7, "get_item", map[string]any{"id": "x", "extra": 1}),
	)
	st := decodeTool(t, resps[1])
	if st.IsError || st.StructuredContent["total"].(float64) != 10 || st.StructuredContent["needs_review"].(float64) != 9 {
		t.Errorf("status = %+v", st.StructuredContent)
	}
	if rq := decodeTool(t, resps[2]); rq.StructuredContent["count"].(float64) != 9 {
	...
	if bq := decodeTool(t, resps[3]); bq.StructuredContent["count"].(float64) != 1 {
		t.Errorf("delivered queue count = %v", bq.StructuredContent["count"])
	}
	item := decodeTool(t, resps[4])
	if item.IsError || item.StructuredContent["queue"] != "delivered" {
...
func TestWriteToolsAreOptIn(t *testing.T) {
	...
	if routed.IsError || routed.StructuredContent["queue"] != "coordinator-review" || routed.StructuredContent["needs_review"] != true {
...
func TestApproveToolRequiresNote(t *testing.T) {
	...
	resps := session(t, srv, initMsg,
		...
		call(3, "approve", map[string]any{"id": id, "note": "coordinator checked with Mina: layout is final", "readiness": "ready_for_next_stage"}),
	...
	if ov.IsError || ov.StructuredContent["queue"] != "animation" {
