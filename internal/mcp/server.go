...
func (s *Server) dispatch(ctx context.Context, req request) (any, *rpcError) {
	...
	case "initialize":
		...
		return map[string]any{
			...
			"serverInfo":      map[string]any{"name": "amberlight-icr", "version": s.Version},
			"instructions": "Amberlight Animation shot-handoff triage pipeline. The model judges readiness; the next stage " +
				"is always looked up from the configured stage order. Read tools are always available. Write tools are " +
				"enabled individually by the operator; routing always follows the configured rules, blocked shots always " +
				"escalate to a supervisor, and low-confidence handoffs stay with a human.",
		}, nil
	case "ping":
...
var idArg = map[string]any{"id": str("Item ID, e.g. AL-1A2B3C4D")}
...
var tools = []tool{
	...
	},
	{
		...
		Description: "Full record for one shot handoff: normalized handoff, readiness verdict, stage lookup and routing decision, review and audit trail.",
	...
	},
	{
		...
		Description: "List items, optionally filtered by queue name (e.g. supervisor-review, coordinator-review, on-hold, a stage name such as lighting or delivered, or artist:<name>) or status. Omit both for everything waiting on human review.",
		InputSchema: obj(map[string]any{
	...
	},
	{
		...
		Description: "Re-apply the configured routing rules to a classified item and run the resulting actions. Cannot bypass confidence gates or the stage order: low-confidence items still go to a coordinator and blocked shots to a supervisor.",
		InputSchema: obj(idArg, "id"),
	...
	},
	{
		...
		Description: "Re-run the readiness check on a stored item and re-route it. Actions already taken for the item are not repeated.",
		InputSchema: obj(idArg, "id"),
	...
	{
		Name: "approve", Title: "Approve / override readiness", write: true,
		Description: "Record a review decision on an item: approve the model's readiness verdict, or override it. The item is then routed as human-verified (the next stage still comes from the stage order). Only enable this for an agent acting on explicit instructions from a human reviewer.",
		InputSchema: obj(map[string]any{
			"id":        idArg["id"],
			"note":      str("Why — recorded in the audit trail"),
			"readiness": map[string]any{"type": "string", "enum": []string{"ready_for_next_stage", "needs_revision", "blocked"}, "description": "Override: corrected readiness (omit to approve as-is)"},
		}, "id", "note"),
		Annotations: map[string]any{"readOnlyHint": false, "destructiveHint": false},
		run: func(ctx context.Context, s *Server, raw json.RawMessage) (any, error) {
			var a struct{ ID, Note, Readiness string }
			if err := decode(raw, &a); err != nil || a.ID == "" {
				return nil, errArgs("id is required")
			}
			...
			return s.Svc.Override(ctx, a.ID, s.actor(), a.Readiness, a.Note)
		},
	},
}
...
func summarize(items []store.Item) []itemSummary {
	...
	for _, it := range items {
		s := itemSummary{ID: it.ID, ShotID: it.Handoff.ShotID, Artist: it.Handoff.Artist, CurrentStage: it.Handoff.CurrentStage,
			Status: string(it.Status), Queue: it.Queue, NeedsReview: it.NeedsReview}
		if c := it.Classification; c != nil {
			s.Readiness, s.Confidence = c.Readiness, c.Confidence
		}
		if d := it.Decision; d != nil {
			s.NextStage, s.Advanced = d.NextStage, d.Advanced
		}
		out = append(out, s)
	}
	return out
