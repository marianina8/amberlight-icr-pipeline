...
// Item is one shot handoff and everything that has happened to it.
type Item struct {
	ID             string                   `json:"id"`
	Handoff        ingest.Handoff           `json:"handoff"`
...
func (f Filter) match(it Item) bool {
	...
	if f.Workspace != "" && it.Handoff.Workspace != f.Workspace {
		return false
	}
...
func sortItems(items []Item) {
	sort.SliceStable(items, func(i, j int) bool {
		if !items[i].Handoff.ReceivedAt.Equal(items[j].Handoff.ReceivedAt) {
			return items[i].Handoff.ReceivedAt.Before(items[j].Handoff.ReceivedAt)
		}
