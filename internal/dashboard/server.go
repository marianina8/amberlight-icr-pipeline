// Package dashboard is the human-in-the-loop web UI: submit a shot handoff,
// watch it get classified and routed, review the queue, approve or override. The
...
type Options struct {
	...
	Examples []Example
	// BasePath mounts the UI under a prefix, e.g. "/demos/amberlight" when it
	...
	// visitors never see each other's handoffs. Requires
...
func New(opt Options) (*Server, error) {
	...
	funcs := template.FuncMap{
		...
		"join": strings.Join,
		// readiness renders a verdict label for people: "ready for next stage".
		"readiness": func(s string) string { return strings.ReplaceAll(s, "_", " ") },
	}
...
func (s *Server) index(w http.ResponseWriter, r *http.Request) {
	...
	// newest handoffs first in the "All" table
	for i, j := 0, len(all)-1; i < j; i, j = i+1, j-1 {
		all[i], all[j] = all[j], all[i]
	}
	p := s.page("Review queue")
	if pending {
		p.Refresh = 3 // handoffs still being classified
	}
...
func (s *Server) getOwned(r *http.Request, id string) (store.Item, error) {
	...
	if it.Handoff.Workspace != workspace(r) && workspace(r) != "" {
...
func (s *Server) item(w http.ResponseWriter, r *http.Request) {
	...
	s.render(w, "item.html", itemData{
		Page: p, Item: it,
		Readiness: s.opt.Svc.Cfg.ReadinessNames(),
...
func (s *Server) override(w http.ResponseWriter, r *http.Request) {
	...
	it, err := s.opt.Svc.Override(r.Context(), id, "dashboard:"+reviewer, r.FormValue("readiness"), r.FormValue("note"))
...
func (s *Server) after(w http.ResponseWriter, r *http.Request, id string, it store.Item, err error, verb string) {
	...
	default:
		http.Redirect(w, r, s.url("/?msg=")+url.QueryEscape(fmt.Sprintf("%s %s → %s", verb, id, it.Queue)), http.StatusSeeOther)
	}
}
