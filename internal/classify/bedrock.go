...
// Bedrock judges handoff readiness with a single Bedrock Converse call.
// Select it with classifier.provider: bedrock (or -classifier bedrock); the
// deployed worker Lambda uses it by default.
type Bedrock struct {
	client ConverseAPI
	cfg    config.Bedrock
	prompt *Prompt
...
func NewBedrock(client ConverseAPI, c *config.Config) (*Bedrock, error) {
	...
	return &Bedrock{client: client, cfg: c.Classifier.Bedrock, prompt: p, labels: c.ReadinessNames(), now: time.Now}, nil
}
...
func (b *Bedrock) Classify(ctx context.Context, h ingest.Handoff) (Classification, error) {
	sys, err := b.prompt.System()
	if err != nil {
		return Classification{}, err
	}
	usr, err := b.prompt.User(h)
	if err != nil {
		return Classification{}, err
	}
	...
	c, err := ParseModelOutput(text.String(), b.labels)
	if err != nil {
		return Classification{}, err
	}
	c.Classifier = b.Name()
