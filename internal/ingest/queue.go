...
type Queue interface {
	Send(ctx context.Context, t Handoff) error
...
// Message is a queued handoff plus delivery metadata.
type Message struct {
...
func (q *DirQueue) Send(_ context.Context, t Handoff) error {
	return q.write("pending", Message{Handoff: t, EnqueuedAt: q.now().UTC()})
}
...
func (q *DirQueue) write(sub string, m Message) error {
	...
	name := fmt.Sprintf("%020d-%s.json", m.EnqueuedAt.UnixNano(), m.Handoff.ID)
