...
type Sink func(ctx context.Context, payload []byte, source string) (Handoff, bool, error)
...
// shot handoff notes never need it).
const MaxWebhookBody = 256 << 10

// WebhookHandler is the local stand-in for API Gateway + the webhook Lambda:
// POST a handoff payload as JSON, get 202 + the handoff ID back.
func WebhookHandler(sink Sink) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("POST /handoffs", func(w http.ResponseWriter, r *http.Request) {
