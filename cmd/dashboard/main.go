// Command dashboard serves the web UI locally: submit a shot handoff and watch it
// get classified and routed, review the human queue, approve or override.
// By default it uses the local data dir (handoffs are processed immediately,
...
import (
	...
	"github.com/marianina8/amberlight-icr-pipeline/internal/dashboard"
...
func main() {
	var (
		...
		queueURL = flag.String("queue-url", os.Getenv("ICR_QUEUE_URL"), "SQS queue (HandoffQueueUrl output) so -store dynamo can accept submissions")
		profile  = flag.String("profile", os.Getenv("AWS_PROFILE"), "AWS CLI profile for -store dynamo")
		examples = flag.String("examples", "demo/handoffs", "directory of example handoffs for the submit page")
		basePath = flag.String("base-path", "", "serve under a path prefix, e.g. /demos/amberlight")
